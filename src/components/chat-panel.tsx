...
import { Send, MoreVertical, BellOff, Bell, Ban, Trash2, ShieldOff, Loader2 } from "lucide-react";
...
export function ChatPanel({ open, onOpenChange, currentUserId, otherUserId, otherName, otherAvatarUrl }: Props) {
  ...
  const toggleMute = async () => {
    ...
    const { error } = await supabase
      ...
      .upsert(
        { user_id: currentUserId, peer_id: otherUserId, muted_at: next },
  ...
  const clearConversation = async () => {
    ...
    const { error } = await supabase
      .from("user_chat_settings")
      .upsert(
        { user_id: currentUserId, peer_id: otherUserId, cleared_at: now },
        { onConflict: "user_id,peer_id" },
      );
    if (error) return toast.error(error.message);
    setClearedAt(now);
